...
//    }
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http.csrf().disable().authorizeHttpRequests().requestMatchers("/home","/user").permitAll().and()
                .authorizeHttpRequests().requestMatchers("/admin/**").hasAnyRole("ADMIN").anyRequest().authenticated()
                .and().userDetailsService(myUserDetailsService).formLogin().and().build();
    }
    @Bean
    public WebSecurityCustomizer webSecurityCustomizer() {     
        return(web)->web.ignoring().requestMatchers("/feedback.html","/script/feedbackquestion.js","/submitFeedback","/giveFeedback","/registrationsuccessfull.html");
    }
